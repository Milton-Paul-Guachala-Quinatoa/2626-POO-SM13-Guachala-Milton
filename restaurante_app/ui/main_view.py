...
class MainView(tk.Frame):
    ...
    def definir_estilos(self):
        ...
        estilo.configure("MenuApp.TButton", background=self.color_secundario,
        ...
        estilo.configure("CerrarSesion.TButton", background="#e11d48", foreground="#ffffff",
                         font=("Arial", 10, "bold"), padding=(12, 8), borderwidth=0)
    ...
    def construir_interfaz(self):
        ...
        tk.Label(encabezado, text="RESTAURANTE APP", bg=self.color_encabezado, fg="#ffffff",
                 font=("Arial", 19, "bold")).pack(anchor="w")
        tk.Label(encabezado, text=f"Bienvenido, {self.usuario_actual.nombre}",
                 bg=self.color_encabezado, fg="#dbeafe", font=("Arial", 11)).pack(anchor="w", pady=(6, 0))
        ...
        ttk.Button(barra, text="Cerrar sesion", command=self.cerrar_sesion,
                   style="CerrarSesion.TButton").pack(side="right")
    ...
    def crear_boton_menu(self, contenedor, texto, comando):
        ttk.Button(contenedor, text=texto, command=comando,
                   style="MenuApp.TButton").pack(side="left", padx=(0, 8))
    ...
    def mostrar_inicio(self):
        ...
        tk.Label(self.contenido, text="Panel principal", bg=self.color_fondo,
                 fg=self.color_encabezado, font=("Arial", 18, "bold")).pack(anchor="w", pady=(0, 8))
        tk.Label(self.contenido,
                 text="Seleccione una opcion superior para visualizar la informacion cargada.",
                 bg=self.color_fondo, fg=self.color_texto, font=("Arial", 12)).pack(anchor="w")

    def mostrar_productos(self):
        self.limpiar_contenido()
        self.crear_titulo_seccion("Productos registrados")
        for producto in self.restaurante_servicio.listar_productos():
            texto = (f"{producto.codigo} - {producto.nombre} | "
    ...
    def mostrar_usuarios(self):
        ...
        for usuario in self.restaurante_servicio.listar_usuarios():
            texto = f"{usuario.identificacion} - {usuario.nombre} | correo: {usuario.correo}"
            self.crear_fila_informacion(texto)

    def crear_titulo_seccion(self, texto):
        tk.Label(self.contenido, text=texto, bg=self.color_fondo,
                 fg=self.color_encabezado, font=("Arial", 17, "bold")).pack(anchor="w", pady=(0, 14))

    def crear_fila_informacion(self, texto):
        fila = tk.Frame(self.contenido, bg="#ffffff", padx=14, pady=10)
        fila.pack(fill="x", pady=(0, 8))
        tk.Label(fila, text=texto, bg="#ffffff", fg=self.color_texto,
                 font=("Arial", 11)).pack(anchor="w")

    def crear_barra_estado(self):
        barra_estado = tk.Frame(self, bg=self.color_secundario, padx=18, pady=8)
        barra_estado.pack(fill="x", side="bottom")
        tk.Label(barra_estado,
                 ...
                 bg=self.color_secundario, fg=self.color_texto, font=("Arial", 10)).pack(side="left")

    def mostrar_funcionalidad_pendiente(self):
        messagebox.showinfo("Proximamente",
                            "La funcionalidad de Ventas se incorporara posteriormente.")
